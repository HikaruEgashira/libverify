...
Four-crate workspace. `libverify-core` is pure logic (serde only);
`libverify-policy` adds OPA; `libverify-output` adds formatters;
`libverify-verif` is verification-only (excluded from normal builds).

```
libverify-core       ← pure evidence model + controls + assessment
    ↑
libverify-policy     ← OPA Rego profiles (regorus)
    ↑
libverify-output     ← SARIF / JSON rendering

libverify-verif      ← Creusot specs (independent, not linked at runtime)
```
